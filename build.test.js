...
describe('Connection Tests', () => {
    ...
    beforeEach(() => {
        connection = new Connection('localhost', 1234, 1000); // Beispielhafte Cache-Dauer von 1000 ms
        connection.connect();
    });

    test('should create and close a new connection', () => {
        expect(connection).toBeInstanceOf(Connection); // Überprüfen, ob eine Verbindung erstellt wurde
        connection.close();
        expect(connection.conn).toBeNull(); // Überprüfen, ob die Verbindung geschlossen wurde
    });

    test('should establish a connection successfully', async () => {
        // Mock für das 'connect'-Ereignis
        const mockOnConnect = jest.fn((event, callback) => {
            if (event === 'connect') {
                process.nextTick(callback); // Simuliere 'connect'-Ereignis sofort
            }
        });
    
        // Mock für 'createConnection' mit 'on' Methode
        net.createConnection.mockImplementation(() => ({
            on: mockOnConnect,
            end: jest.fn(), // Hier hinzufügen, um die 'end'-Methode zu mocken
        }));
    
        // Verwenden Sie 'await' für die asynchrone 'connect'-Methode
        await expect(connection.connect()).resolves.toBeUndefined();
        expect(net.createConnection).toHaveBeenCalledWith({ host: 'localhost', port: 1234 });
        expect(mockOnConnect).toHaveBeenCalledWith('connect', expect.any(Function));
        connection.close(); // Aufräumen nach jedem Testfall, Verbindung schließen
    });

    test('should throw error when connection fails', async () => {
        // Mock für das 'error'-Ereignis
        const mockOnError = jest.fn((event, callback) => {
            if (event === 'error') {
                process.nextTick(callback, new Error('Connection error')); // Simuliere 'error'-Ereignis mit Fehler
            }
        });
        ...
        // Verwenden Sie 'await' für die asynchrone 'connect'-Methode
        await expect(connection.connect()).rejects.toThrow('Connection error');
        expect(net.createConnection).toHaveBeenCalledWith({ host: 'localhost', port: 1234 });
        expect(mockOnError).toHaveBeenCalledWith('error', expect.any(Function));
        connection.close(); // Aufräumen nach jedem Testfall, Verbindung schließen
    });
